...
// ------------------------------------------------------------------------------------------------
      ...
      for(let theta=-180;theta<=180;theta+=(360/BulletNum) ){
      ...
      // 把角度轉換成徑度方便電腦運算
        ...
        let newX = acc.x * Math.cos(radians) - acc.y * Math.sin(radians);
        let newY = acc.x * Math.sin(radians) + acc.y * Math.cos(radians);

      // 高中數學公式


        let dir = new Vector(newX,newY);
        let bullet = new SmallFireBall(this.pos.x, this.pos.y, power, dir);
        entityManager.add(bullet);
      }
...
// ------------------------------------------------------------------------------------------------
      ...
      // let radians1 = theta1 * Math.PI / 180;
      // let newX1 = acc.x * Math.cos(radians1) - acc.y * Math.sin(radians1);
      // let newY1 = acc.x * Math.sin(radians1) + acc.y * Math.cos(radians1);


      // let radians2 = theta2 * Math.PI / 180;
      // let newX2 = acc.x * Math.cos(radians2) - acc.y * Math.sin(radians2);
      // let newY2 = acc.x * Math.sin(radians2) + acc.y * Math.cos(radians2);

      // const dir1 = new Vector(newX1,newY1);
      // const dir2 = new Vector(newX2,newY2);

      // const bullet = new SmallFireBall(this.pos.x, this.pos.y, power, acc);
      // const bullet2 = new SmallFireBall(this.pos.x, this.pos.y, power, dir1);
      // const bullet3 = new SmallFireBall(this.pos.x, this.pos.y, power, dir2);
      // entityManager.add(bullet);
